...
项目重点不只是抓取网页，而是建立一套可审计的数据采集流程：每条数据都需要保留来源 URL、站点 robots 校验结果、抓取时间、正文版本、适用范围、政策主题、原文证据和解析状态，确保数据来源真实、规则可追踪、采集行为合规。
...
- 严格遵守目标站点 `robots.txt`、公开访问规则、合理限速和来源标注要求。
...
| `robots_allowed` | boolean | robots 校验是否允许抓取 |
| `robots_checked_at` | datetime | robots 校验时间 |
| `title` | string | 页面标题 |
...
1. 每个域名首次采集前请求 `https://domain/robots.txt`。
2. 使用标准 robots 解析器判断目标 URL 是否允许当前 User-Agent 抓取。
3. 缓存 robots 结果，记录 `robots_checked_at`、规则摘要和是否允许。
4. 若 robots 明确禁止目标路径，系统跳过该 URL，并记录跳过原因。
5. 若 robots 文件不可访问，按保守策略处理：降低频率，只抓取明确公开、无需登录、非敏感页面；必要时进入人工审核队列。
6. 禁止抓取搜索结果页、登录页、购物下单页、个人信息页、运单跟踪页、价格实时查询接口等高敏或非政策类页面。
...
- 标准浏览器 User-Agent：仅在页面对普通 HTTP 客户端返回异常、且 robots 允许访问时使用。
...
2. 对每个域名执行 robots 校验。
3. 抓取政策入口页、服务条款页、帮助中心页、PDF 文档和公告列表。
4. 提取候选链接，只保留政策规则相关 URL。
5. 对候选 URL 再次执行 robots 校验和去重。
6. 下载 HTML/PDF，保存原始文件。
7. 抽取正文，识别标题、更新时间、正文段落、表格和附件。
8. 根据关键词和规则模型标注政策主题。
9. 生成结构化 JSONL 和 CSV。
10. 输出采集报告：成功、跳过、失败、robots 禁止、待人工审核。
...
- 复杂判断、正则表达式、robots 处理、限速、保险赔付解析、国际术语映射等位置必须写中文注释，解释为什么这样处理。
...
- 对外部站点请求必须通过统一的 `fetcher.py`，确保 robots、限速、User-Agent、重试和日志规则不会被绕开。
...
- `data/logs/robots_report.csv`：robots 校验结果。
...
| M2 | 合规抓取框架 | robots 校验、限速、请求器、任务队列 |
...
| 风险 | 影响 | 应对 |
| --- | --- | --- |
| robots 禁止部分路径 | 数据覆盖不足 | 跳过禁止路径，转向官方 PDF、公告、帮助中心允许路径或人工整理 |
| 页面动态渲染 | 普通请求无法提取正文 | 在 robots 允许前提下使用 Playwright 低频渲染 |
