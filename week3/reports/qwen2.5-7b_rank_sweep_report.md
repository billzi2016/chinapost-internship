...
基础配置使用 `configs/qwen2.5-7b-lora.yaml`。本次 run 的 `chunk-iters=100`，基础总步数为 1000，因此 rank 1、2、4、8、16 都完整训练到了 1000 step；rank 32 在 700 step 触发 gate，提前停止。

实验产物目录：

```text
../mlx_qwen_sft/runs/20260625_233007_qwen2.5-7b-lora_rank_sweep/
```
...
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 400 | 1.2375 | 1.2125 | 1.0000 | 0.3333 | 0.0000 | 1.7500 | 2.3750 |
| 2 | 1000 | 3.3000 | 3.3000 | 1.0000 | 0.3333 | 0.0000 | 2.0000 | 2.5000 |
| 4 | 600 | 3.1000 | 2.4083 | 1.0000 | 0.3333 | 0.0000 | 1.5000 | 1.8750 |
| 8 | 500 | 3.0688 | 3.0250 | 1.0000 | 0.3333 | 0.0000 | 1.6250 | 1.6250 |
| 16 | 200 | 2.9812 | 2.8625 | 1.0000 | 0.3333 | 0.0000 | 1.3750 | 1.3750 |
| 32 | 200 | 2.8000 | -0.3188 | 1.0000 | 0.3333 | 0.0000 | 0.5000 | 1.1250 |

从结果看，7B 和 3B 的最优 rank 分布并不相同。3B 更偏向小 rank，而这次 7B 最稳定、得分最高的是 rank 2。rank 8 和 rank 16 也保持了较好的 final score。rank 1 虽然没有明显格式崩坏，但综合分偏低；rank 32 在 700 step 因 JSON 崩坏被 gate 提前停止。
...
![Rank 1 dashboard](../mlx_qwen_sft/runs/20260625_233007_qwen2.5-7b-lora_rank_sweep/plots/qwen2.5-7b-lora-r1_training_dashboard.jpg)

结论：rank 1 可以用，但不是这次 7B 实验的优先方案。它偏保守，业务表达不差，但综合得分和通用任务纯净度不如更优 rank。

### Rank 2

rank 2 的 best step 出现在 1000 step，best score 和 final score 都是 3.3000，是本次 7B sweep 中表现最好的配置。它说明模型并没有在后期退化，反而在完整训练结束时达到最佳点。

从指标上看，rank 2 的 `postal_terms=2.0000`、`next_steps=2.5000`，兼顾了业务词覆盖和处理建议；JSON 可解析率保持为 1.0，JSON 必需字段完整率虽然仍只有 0.3333，但没有额外污染或风险惩罚。

训练图：

![Rank 2 dashboard](../mlx_qwen_sft/runs/20260625_233007_qwen2.5-7b-lora_rank_sweep/plots/qwen2.5-7b-lora-r2_training_dashboard.jpg)
...
![Rank 4 dashboard](../mlx_qwen_sft/runs/20260625_233007_qwen2.5-7b-lora_rank_sweep/plots/qwen2.5-7b-lora-r4_training_dashboard.jpg)
...
![Rank 8 dashboard](../mlx_qwen_sft/runs/20260625_233007_qwen2.5-7b-lora_rank_sweep/plots/qwen2.5-7b-lora-r8_training_dashboard.jpg)
...
![Rank 16 dashboard](../mlx_qwen_sft/runs/20260625_233007_qwen2.5-7b-lora_rank_sweep/plots/qwen2.5-7b-lora-r16_training_dashboard.jpg)

结论：rank 16 不算最优，但它证明 7B 不像 3B 那样对高 rank 非常敏感。它可以作为“更高容量但仍稳定”的参考点。

### Rank 32

rank 32 的 best step 出现在 200 step，best score 为 2.8000，但最终在 700 step 触发 gate 停止，final score 为 -0.3188。停止原因是：

```text
JSON 大面积不可解析
```

这次退化比 3B 更明确，因为 7B 的 rank 32 在 700 step 直接被 gate 拦停，而不是硬跑到结束。final 阶段 `json_valid` 已经掉到 0，`json_keys` 也掉到 0，说明输出格式彻底失控。

训练图：

![Rank 32 dashboard](../mlx_qwen_sft/runs/20260625_233007_qwen2.5-7b-lora_rank_sweep/plots/qwen2.5-7b-lora-r32_training_dashboard.jpg)
...
第三，格式输出仍然是核心风险。虽然 rank 2、8、16 都保持了 JSON 可解析率 1.0，但 rank 4 在后期已经出现 `json_valid` 下滑，rank 32 则直接在 700 step 因“JSON 大面积不可解析”被 gate 拦截。也就是说，7B 的业务能力更强，但格式崩坏仍然是必须监控的失效模式。
...
```text
../mlx_qwen_sft/runs/20260625_233007_qwen2.5-7b-lora_rank_sweep/rank_2/best_adapter/qwen2.5-7b-lora-r2/
```
...
```text
rank 32：700 step 因 JSON 大面积不可解析被 gate 停止
