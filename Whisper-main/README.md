...
python main.py --language zh         # 强制按中文转录，适合中文视频被误判成英文
...
| `--language` | 自动检测 | 指定转录语言，例如 `zh`；传 `zh` 时会强制按中文转录 |
...
如果视频主体是中文，但 Whisper 自动识别时容易误判成英文，建议直接指定：

```bash
python main.py --language zh
```

这样会统一按中文转录。即使内容里夹杂少量英文，也仍然以中文语境进行识别。
...
If you only want the original transcription without translation, use:
