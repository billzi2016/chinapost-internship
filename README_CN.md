...
补充说明：本仓库也是一个复现整理后的项目版本。目录中的 `week1`、`week2`、`week3` 以及后续阶段名称，主要对应项目推进顺序，不与真实日历周严格一一对应。这里去除了人工调参、样本筛选、验证数据集整理、入职培训、会议沟通等非主线开发时间，因此这些 `week` 更适合理解为任务阶段，而不是自然周时间。
