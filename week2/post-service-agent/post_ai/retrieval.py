...
class FaissPostalIndex:
    ...
    def save(self, artifact_dir: Path) -> None:
        ...
        faiss.write_index(self.index, str(artifact_dir / "postal.faiss"))
        payload = {
            "embedding_model": self.embedding_model,
            "provider": self.provider,
            "documents": [document.model_dump() for document in self.documents],
        }
        (artifact_dir / "postal_metadata.json").write_text(
    ...
    def load(cls, artifact_dir: Path) -> "FaissPostalIndex":
        index = faiss.read_index(str(artifact_dir / "postal.faiss"))
        payload = json.loads((artifact_dir / "postal_metadata.json").read_text(encoding="utf-8"))
