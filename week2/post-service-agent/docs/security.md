...
在当前已经实现的代码路径下，数据库访问基本都通过 Django ORM、django-ninja schema 和 `pgvector.django` 表达式完成，没有把用户输入拼接进 SQL 字符串，因此 SQL 注入风险已经很低，基本无风险。

这不表示未来新增功能可以忽略边界。后续如果新增功能，仍然要遵守：
